...
for i in range(10):
    ...
    pie_start = i * 10
    pie_end = 270 + i * 10
    rect_cord = (10, 10, 22, 22)

    img_in = Image.new("RGB", img_size, (i * 20, 255 - i * 20, abs(255 - i * 50)))
    draw_in = ImageDraw.Draw(img_in)
    draw_in.pieslice(pie_cord, start=pie_start, end=pie_end, fill=(0, 120, 120))
    draw_in.rectangle(rect_cord, fill=(255, 0, 255))
    img_in.save(f"dataset/images/segmentation/{i:04}.png")

    img_out = Image.new("RGB", img_size, (0, 255, 0))
    draw_out = ImageDraw.Draw(img_out)
    draw_out.pieslice(pie_cord, start=pie_start, end=pie_end, fill=(0, 0, 255))
    draw_out.rectangle(rect_cord, fill=(255, 0, 0))
