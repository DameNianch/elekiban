...
train_faucet = elekiban.pipeline.toolbox.SimpleFaucet([input_pipeline], [output_pipeline], batch_size=7)
valid_faucet = elekiban.pipeline.toolbox.SimpleFaucet([input_pipeline], [output_pipeline], batch_size=7)


model = elekiban.model.image.vectorization.get_efficient_net_v2("image_input", "label_output", 3)
output_path = elekiban.executor.train(model, train_faucet, valid_faucet, epochs=150, output_path="output")
print(output_path)
